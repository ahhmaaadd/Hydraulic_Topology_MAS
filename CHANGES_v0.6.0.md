...
graceful degradation, and both paths reach the same certificate through the same
graph, so comparing them is controlled.

*(Corrected in v0.7.0: this was originally described as "the A0 control arm",
which is wrong. The deterministic planner is symbolic-without-neural — arm A2.
A0 is neural-without-symbolic and did not exist until v0.7.0.)*
