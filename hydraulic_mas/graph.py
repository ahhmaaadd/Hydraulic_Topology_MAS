...
from .research_guards import derive_research_need_hints, enforce_coverage_gate, enforce_minimum_plan
from .schemas import (
...
from .search import SearchClient, TavilySearchClient
...
class Workflow:
    ...
    def route_after_requirements_critique(self, state: OverallState) -> Literal["clarify_requirements", "finalize_requirements"]:
    ...
    def plan_research(self, state: OverallState) -> dict[str, Any]:
        ...
        max_initial = min(state.get("max_searches", self.settings.max_searches), max(12, len(hints)))
    ...
    def web_research_worker(self, state: OverallState) -> dict[str, Any]:
        ...
        error: str | None = None
        try:
            results = self.search_client.search(task["query"], max_results=self.settings.max_results_per_search)
        except Exception as exc:  # A failed query is evidence for the coverage gate, not a silent crash.
            results = []
            error = f"{type(exc).__name__}: {exc}"

        record = SearchRecord(task_id=task["id"], query=task["query"], results=results, error=error)
        if error:
            finding = TaskFinding(
                task_id=task["id"],
